/*
Program Name: fliplr.c
Programmer's Name: Sparsh Jain
Program Description: To create a left to right flipped image of a given image
Date: 05/10/2016
*/
		...
		for(h = 0; h < height; h++)
			{
				for(w = 0; w < (width/2); w++)
					{
						temp = IMG[h][w];
						IMG[h][w] = IMG[h][width - (w + 1)];
						IMG[h][width - (w + 1)] = temp;
					}					
			}
		
		printf("Enter the name for the left to right flipped image file : ");
		scanf(" %s", image);		
